...
void mouseClicked() {
  ...
  for (int x = 0; x < levels[level][floor(mouseY/64)].length(); x++) {
    if (x == floor(mouseX/64) || x == floor(mouseX/64)+1) {
      k=k+"g";
    } else {
  ...
  k="";
  for (int x = 0; x < levels[level][floor(mouseY/64)+1].length(); x++) {
    if (x == floor(mouseX/64) || x == floor(mouseX/64)+1) {
      k=k+"g";
    } else {
      k=k+str(levels[level][floor(mouseY/64)+1].charAt(x));
    }
  }
  levels[level][floor(mouseY/64)+1]=k;
  loadTileset(levels[level]);
}
